...
[private]
@canonicalize path="":
    uvx python -c "from pathlib import Path; import sys; path = Path(sys.argv[1]); matches = list(path.parent.glob(path.name)) if '*' in path.name else [path]; print(matches[0].resolve().as_posix())" "{{ path }}"
